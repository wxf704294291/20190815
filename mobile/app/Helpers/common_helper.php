...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, $order_type = 0, $deposit_fee = 0)
{
	...
	if ($is_go && ($user_money || $frozen_money || $rank_points || $pay_points)) {
		...
		//不想更新组的填写在这里,用逗号隔开，如：13,3,26
		$rank_group = '13,3';
		
		$sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . 'SET user_rank = \'' . $rank_row['rank_id'] . ('\' WHERE user_id = \'' . $user_id . '\' AND  user_rank NOT IN('.$rank_group.')');
		$GLOBALS['db']->query($sql);
		$sql = 'UPDATE ' . $GLOBALS['ecs']->table('sessions') . 'SET user_rank = \'' . $rank_row['rank_id'] . '\', discount= \'' . $rank_row['discount'] . ('\' WHERE userid = \'' . $user_id . '\' AND adminid = 0 AND  user_rank NOT IN('.$rank_group.')');
