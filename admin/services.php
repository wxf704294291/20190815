...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        if (isset($_POST['remove']))
        {
         ...
         if($_POST['extension_code']=='deleted'){
			 $sql="DELETE from ". $ecs->table('im_service') . "WHERE id " . db_create_in($ids); 
		 }else{
			 $sql = "UPDATE". $ecs->table('im_service') . " SET status = 0  WHERE id " . db_create_in($ids); 
		 }   
            $res = $db->query($sql); 
...
function services_list(){
    ...
    $sqlCount  = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('im_service') . ' s'
        . " INNER JOIN " . $GLOBALS['ecs']->table('admin_user') . ' a on a.user_id = s.user_id'
        . " INNER JOIN " . $GLOBALS['ecs']->table('seller_shopinfo') . ' r on r.ru_id = a.ru_id'
        .' WHERE s.id <> 0 ';
