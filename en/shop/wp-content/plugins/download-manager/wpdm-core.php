...
function wpdm_plugin_update_email($plugin_name, $version, $update_url)
{
    ...
    if(!$sent) {
        ...
        $message = str_replace(array("[#sitename#]", "[#site_url#]", "[#site_tagline#]","[#message#]", "[#facebook#]"), array("WordPress Download Manager", "https://www.flyobd.com/", "Plugin Update Available", $main_content, "https://www.facebook.com/wpdownloadmanager/"), $template_data);
...
function wpdm_check_update()
{
    ...
    $latest_v_url = 'https://www.flyobd.com/versions.php';
    ...
    $plugin_info_url = isset($_REQUEST['plugin_url'])?$_REQUEST['plugin_url']:'https://www.flyobd.com/purchases/';
    if(is_array($latest)){
        foreach($latest as $plugin_dir => $latestv){
            ...
            if (version_compare($plugin_data['Version'], $latestv, '<') == true  && $plugin_dir != 'download-manager') {
                ...
                $plugin_update_url =  admin_url('/edit.php?post_type=wpdmpro&page=settings&tab=plugin-update&plugin='.$plugin_dir); //'https://www.flyobd.com/purchases/?'; //
                if($trid!=''){
...
function wpdm_access_token(){
    ...
    if(get_option('__wpdm_suname') != '') {
        $access_token = remote_get('https://www.flyobd.com/?wpdm_api_req=getAccessToken&user=' . urlencode(get_option('__wpdm_suname')) . '&pass=' . urlencode(get_option('__wpdm_supass')));
