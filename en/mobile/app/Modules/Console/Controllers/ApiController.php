...
class ApiController extends \App\Modules\Admin\Controllers\EditorController
{
			...
			if (empty($new)) {
				...
				if (!empty($index)) {
					...
					if (empty($new)) {
						...
						$keep = array('ru_id' => 0, 'type' => 'index', 'page_id' => 0, 'title' => 'Homepage', 'data' => $index, 'default' => 1, 'review_status' => 3, 'is_show' => 1);
		...
		if (IS_POST) {
			...
			if (empty($view)) {
				...
				if (empty($new)) {
					...
					$view = array('ru_id' => 0, 'type' => 'index', 'title' => 'Homepage', 'data' => $data, 'default' => 1);
	...
	public function actionOldSave()
	{
		if (IS_POST) {
			$time = gmtime();
			$keep = array('ru_id' => 0, 'title' => 'Homepage', 'type' => 'index', 'data' => !empty($_POST['data']) ? $_POST['data'] : '', 'update_at' => $time, 'default' => 1);
	...
	public function actionRestore()
	{
		$ru_id = input('ruid');
		$data = str_replace('<?php exit("no access");', '', file_get_contents(ROOT_PATH . 'storage/app/diy/default.php'));
		$keep = array('type' => 'index', 'title' => 'Homepage', 'data' => $data);
