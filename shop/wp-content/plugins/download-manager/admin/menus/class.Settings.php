...
class Settings
{
    ...
    function pluginUpdate(){
        ...
        if(isset($_POST['__wpdm_suname']) && $_POST['__wpdm_suname'] != ''){
            ...
            if($access_token != '') {
                $purchased_items = remote_get('https://www.flyobd.com/?wpdmppaction=getpurchaseditems&wpdm_access_token=' . $access_token);
        ...
        if(get_option('__wpdm_suname') != '') {
            ...
            if(!$purchased_items || wpdm_query_var('newpurchase') != '' ) {
                $purchased_items = remote_get('https://www.flyobd.com/?wpdmppaction=getpurchaseditems&wpdm_access_token=' . wpdm_access_token());
                update_option('__wpdm_purchased_items', $purchased_items);
            }
        ...
        if(get_option('__wpdm_freeaddons') == '' || wpdm_query_var('newpurchase') != '' || 1) {
            $freeaddons = remote_get('https://www.flyobd.com/?wpdm_api_req=getPackageList&cat_id=1148');
