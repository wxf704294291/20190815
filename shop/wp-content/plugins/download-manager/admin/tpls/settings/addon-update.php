        ...
        <?php if(get_option('__wpdm_suname') =='') { ?>
            <div class="panel-body">
                <div class="form-group">
                    <em><?php echo sprintf(__( "Enter your %s login info" , "download-manager" ), '<a href="https://www.flyobd.com/" target="_blank">wpdownloadmanager.com</a>'); ?>:</em>
                </div>
                <div class="form-group">
                                    ...
                                    foreach($item->download_url as $file => $dlu){
                                        ...
                                        if(!strstr($plugin_name, "download-manager-")){
                                            ...
                                            <tr class="<?php if($item->order_status == 'Expired'){  ?>bg-danger<?php } else { ?><?php echo version_compare($latest[$plugin_name], $plugin_data['Version'], '>')?'bg-warning':(!$plugin_data?'':'bg-success'); ?><?php } ?>">
                                                <td><a href="https://www.flyobd.com/?p=<?php echo $item->pid; ?>" target="_blank"><?php echo $item->post_title; ?> ( <?php echo basename($file); ?> )</a></td>
                                                ...
                                                <td style="width: 100px">
                                                    ...
                                                    <?php } else { ?>
                                                        <a href="https://www.flyobd.com/user-dashboard/purchases/order/<?php echo $item->oid; ?>/" target="_blank" class="btn btn-xs btn-danger btn-block"><?php _e('Expired','wpdmpro'); ?></a>
                                                    <?php } ?>
                                                </td>
                                            </tr>
                        ...
                        foreach($freeaddons as $addon){
                            ...
                            <tr class="<?php if(isset($latest[$plugin_name])) { echo version_compare($latest[$plugin_name], $plugin_data['Version'], '>')?'bg-warning':(!$plugin_data?'':'bg-success'); } ?>">
                                ...
                                <td style="width: 100px">
                                    ...
                                    <?php if(!$plugin_data){ ?>
                                        <a href="#" data-url="https://www.flyobd.com/?wpdmdl=<?php echo $addon->ID; ?>" data-action="installaddon" data-plugin="<?php echo $plugin_name; ?>" class="btn btn-xs btn-success btn-block btn-update"><i class="fa fa-plus"></i> <?php _e('Install','wpdmpro'); ?></a>
                                    <?php } else if(isset($latest[$plugin_name]) && version_compare($latest[$plugin_name], $plugin_data['Version'], '>')){ ?>
                                        <a href="#" data-url="https://www.flyobd.com/?wpdmdl=<?php echo $addon->ID; ?>" data-action="updateaddon" data-plugin="<?php echo $plugin_name; ?>" class="btn btn-xs btn-warning btn-block btn-update"><i class="fas fa-sync"></i> <?php _e('Update','wpdmpro'); ?></a>
                                    <?php } else echo "<span class='text-success'><i class='fa fa-check-circle'></i> ". __( "Updated" , "download-manager" )."</span>"; ?>
